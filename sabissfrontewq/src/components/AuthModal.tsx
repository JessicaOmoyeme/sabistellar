...
import {
  ...
  type DiscoveredStellarWallet,
  type StellarWalletKind,
} from "../lib/stellar-wallet.ts";
...
function FreighterIcon() {
  return (
    <svg viewBox="0 0 32 32" aria-hidden="true">
      ...
      <text x="16" y="20" text-anchor="middle" fill="#fff" font-size="14" font-weight="bold">
        F
      </text>
    </svg>
  );
...
function LobstrIcon() {
  return (
    <svg viewBox="0 0 32 32" aria-hidden="true">
      ...
      <circle cx="12" cy="16" r="6" fill="#FF5733" />
    </svg>
  );
}
...
function AlbedoIcon() {
  return (
    <svg viewBox="0 0 32 32" aria-hidden="true">
      ...
      <circle cx="16" cy="12" r="2" fill="#1E90FF" />
    </svg>
  );
}
...
function LedgerIcon() {
  return (
    <svg viewBox="0 0 32 32" aria-hidden="true">
      ...
      <line x1="10" y1="12" x2="22" y2="12" stroke="#fff" stroke-width="1" />
      <line x1="10" y1="16" x2="22" y2="16" stroke="#fff" stroke-width="1" />
      <line x1="10" y1="20" x2="22" y2="20" stroke="#fff" stroke-width="1" />
    </svg>
  );
}
...
function getFallbackWalletIcon(kind: StellarWalletKind): Component {
  switch (kind) {
    case "freighter":
...
function WalletTileIcon(props: { wallet: DiscoveredStellarWallet }) {
...
export default function AuthModal(props: AuthModalProps) {
  const [wallets, setWallets] = createSignal<DiscoveredStellarWallet[]>([]);
  ...
  const handleWalletClick = async (wallet: DiscoveredStellarWallet) => {
    ...
    try {
      ...
      const account = await requestWalletAccount(walletKit, wallet.kind);
                ...
                <Show
                  ...
                  fallback={
                    <div class="pm-auth-wallet-empty">
                      ...
                      <p>Install Freighter, Lobstr, Albedo, or connect a Ledger wallet.</p>
                    </div>
                  }
                >
