...
test("readProjectedEventDetailView uses cached event markets for instant quote paint", () => {
  ...
  assert.ok(projected);
  assert.equal(projected.selectedMarket.primaryMetric, "24%");
  assert.equal(projected.selectedMarket.meta, "$1.2K vol");
  assert.equal(projected.selectedMarket.headerMeta, "Ends Apr 30");
  assert.equal(projected.selectedMarket.quotes[0]?.centsLabel, "28¢");
  assert.equal(projected.selectedMarket.quotes[1]?.centsLabel, "72¢");
  assert.equal(projected.selectedMarket.actionQuotes[0]?.centsLabel, "28¢");
  assert.equal(projected.selectedMarket.actionQuotes[1]?.centsLabel, "72¢");
...
test("readProjectedEventDetailView preserves enriched home feed odds and volume", () => {
  ...
  assert.ok(projected);
  assert.equal(projected.selectedMarket.primaryMetric, "76%");
  assert.equal(projected.selectedMarket.meta, "$1.2K vol");
  assert.equal(projected.facts.find(fact => fact.label === "Market odds")?.value, "76%");
  assert.equal(projected.facts.find(fact => fact.label === "Buy yes")?.value, "98¢");
  assert.equal(projected.facts.find(fact => fact.label === "Buy no")?.value, "2¢");
...
test("loadEventDetailView uses current_prices before orderbook hydration arrives", async () => {
  ...
  assert.equal(view.selectedMarket.meta, "Volume unavailable");
  assert.equal(view.selectedMarket.quotes[0]?.centsLabel, "3¢");
  assert.equal(view.selectedMarket.actionQuotes[0]?.centsLabel, "3¢");
  assert.equal(view.selectedMarket.quotes[1]?.centsLabel, "97¢");
  assert.equal(view.facts.find(fact => fact.label === "Market odds")?.value, "3%");
  assert.equal(view.facts.find(fact => fact.label === "Buy yes")?.value, "3¢");
  assert.equal(view.facts.find(fact => fact.label === "Buy no")?.value, "97¢");
...
test("hydrateEventDetailView can surface event market prices before slower detail resources finish", async () => {
  ...
  assert.ok(preview);
  assert.equal(preview?.selectedMarket.primaryMetric, "42%");
  assert.equal(preview?.selectedMarket.meta, "$1.2K vol");
  assert.equal(preview?.facts.find(fact => fact.label === "Market odds")?.value, "42%");
  assert.equal(preview?.facts.find(fact => fact.label === "Buy yes")?.value, "3¢");
  assert.equal(preview?.facts.find(fact => fact.label === "Buy no")?.value, "97¢");
  ...
  assert.equal(preview?.selectedMarket.actionQuotes[0]?.centsLabel, "3¢");
